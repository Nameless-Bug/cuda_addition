...
void gemmbatched(cublasHandle_t handle, 
    float** a, int arow, int acol, 
    float** b, int brow, int bcol,
    float** c, int* ptr_crow, int& ptr_ccol, int batch
){
...
int main(){
    ...
    // 定义device内存，并进行复制
    float* a_device[batch] = {nullptr};
    float* b_device[batch] = {nullptr};
    float* c_device[batch] = {nullptr};
    float** device_ptr_array_a = nullptr;
    float** device_ptr_array_b = nullptr;
    float** device_ptr_array_c = nullptr;
    cudaMalloc(&device_ptr_array_a, sizeof(float*) * batch);
    cudaMalloc(&device_ptr_array_b, sizeof(float*) * batch);
    cudaMalloc(&device_ptr_array_c, sizeof(float*) * batch);
    ...
    for(int i = 0; i < batch; ++i){
        cudaMalloc(&a_device[i], a_bytes);
        cudaMalloc(&b_device[i], b_bytes);
        cudaMalloc(&c_device[i], c_bytes);

        cudaMemcpyAsync(a_device[i], a_host, a_bytes, cudaMemcpyHostToDevice, stream);
        cudaMemcpyAsync(b_device[i], b_host, b_bytes, cudaMemcpyHostToDevice, stream);
    }

    cudaMemcpyAsync(device_ptr_array_a, a_device, sizeof(float*) * batch, cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(device_ptr_array_b, b_device, sizeof(float*) * batch, cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(device_ptr_array_c, c_device, sizeof(float*) * batch, cudaMemcpyHostToDevice, stream);
    ...
    cudaMemcpyAsync(c_host, c_device[batch-1], sizeof(c_host), cudaMemcpyDeviceToHost, stream);
